...
8. **Monorepo paths only** — implement under `services/`, `uis/`, `tests/`; no delivery folder.
...
| Path                               | Responsibility                                                       |
| ---------------------------------- | -------------------------------------------------------------------- |
| `services/.../chat/ws.py`          | WebSocket accept, bind `session_id`, route inbound events            |
...
**Acceptable:** FastAPI WebSocket (or stack equivalent) + LangGraph stream mode that yields tokens; one producer task per generation with a cancel handle; subscribers receive identical frames.

**Not acceptable:** SSE-only for this part; buffering full reply then dumping once; client-side “stop” that ignores tokens while the model keeps generating; using only LangGraph HITL `interrupt()` without aborting the stream task; deleting the partial assistant message; empty chat after reconnect; calling the agent once per subscribed supervisor tab; inventing a `parte-2-realtime-ws/` delivery folder.
...
function connect() {
  const ws = new WebSocket(`${base}/ws/chat/${session_id}`);
