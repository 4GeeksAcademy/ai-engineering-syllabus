...
- `POST /auth/change-password` — accepts `{ current_password, new_password }`. Requires a valid session token. Verify the current password before updating. Return `400` if the current password is wrong.
- Integrate one transactional email service to send the reset email. The email must include the reset link and be readable on mobile. The README limits choices to these two because they allow development sending without a custom DNS-verified domain.
