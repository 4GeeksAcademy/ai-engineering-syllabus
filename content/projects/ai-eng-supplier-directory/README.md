...
- [ ] Define the Pydantic `Supplier` model with the required fields from your CONTEXT: name, country, product categories, the rate field defined in your CONTEXT, `updated_at`, and status.
...
- [ ] The rate field defined in your CONTEXT must be a positive number. Pydantic must reject zero or negative values before the data reaches TinyDB.
...
- [ ] Display the full list of suppliers in a table or list with their main fields: name, country, categories, the rate field from your CONTEXT, and status.
...
- [ ] Allow updating the rate field defined in your CONTEXT from the interface and reflect the change in the list immediately after the API responds.
